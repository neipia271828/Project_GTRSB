...
@login_required
def lap_graph():
    return render_template('lap_graph.html')
...
def get_lap_times():
    try:
        laps = LapTime.query.filter_by(user_id=current_user.id).order_by(LapTime.created_at.desc()).all()
        return jsonify([{
            'id': lap.id,
            'game_title_id': lap.game_title_id,
            'car_id': lap.car_id,
            'course_id': lap.course_id,
            'game_title': lap.game_title.name if lap.game_title else '',
            'car': lap.car.name if lap.car else '',
            'course': lap.course.name if lap.course else '',
            'time': lap.time,
            'memo': lap.memo,
            'created_at': lap.created_at.strftime('%Y-%m-%d %H:%M:%S')
        } for lap in laps])
...
def add_lap_time():
    try:
        data = request.get_json()
        if not all(k in data for k in ['game_title_id', 'car_id', 'course_id', 'time']):
            return jsonify({'error': 'すべての項目を入力してください'}), 400
        ...
        db.session.rollback()
        print(f"Error in add_lap_time: {str(e)}")
        return jsonify({'error': 'ラップタイムの記録に失敗しました'}), 500
